 ...
 */
$options = getopt("p:f:i:");

if ( empty($options['f']) || empty($options['i'] )) {
    echo "\nError: Missing option -p or -f, see example command below\n";
    echo "> php uploadFile.php -p parentid -i fileid -f filename\n\n";
    exit;
}
...
$content = downloadFile($service,$fileid);
$fp = fopen(dirname(__FILE__).'/../../backup/'.$filename, 'w');
fwrite($fp, $content);
fclose($fp);